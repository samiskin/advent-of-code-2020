...
console.log(input.filter(([ [lower, upper], letter, pattern]) => {
    const num =  pattern.split('').filter((l) => l === letter).length 
    return [pattern[lower - 1], pattern[upper - 1]].filter((l) => l === letter).length === 1;
