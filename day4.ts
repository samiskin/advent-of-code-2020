...
`.trim().split('\n\n').map((s) => s.split(' ').flatMap((s) => s.split('\n')));
...
const validations: Record<string, (...args: any) => boolean> = {
    byr: (yr: string) => yr.length === 4 && (parseInt(yr) >= 1920 && parseInt(yr) <= 2002),
    iyr: (yr: string) => yr.length === 4 && (parseInt(yr) >= 2010 && parseInt(yr) <= 2020),
    eyr: (yr: string) => yr.length === 4 && (parseInt(yr) >= 2020 && parseInt(yr) <= 2030),
    hgt: (ht: string) => {
        const [n, unit] = [parseInt(ht.slice(0, -2)), ht.slice(-2)];
        return unit === 'cm' ? (n >= 150 && n <= 193) : (n >= 59 && n <= 76);
    },
    ecl: (col: string) => !!['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'].find((c) => c === col),
    hcl: (col: string) => !!col.match(/^#[a-f0-9]{6}$/),
    pid: (id: string) => id.length === 9 && id.split('').every((s) => parseInt(s) !== NaN),
    cid: () => true,
}

console.log(input.filter((entries) => {
    return requiredKeys.every((k) => !!entries.find((s) => {
        const [key, val] = s.split(':');
        return key === k && validations[key] && validations[key](val);
    }))
}).length)
