...
console.log(input.reduce((count, group) => {
    const set = new Set();
    group.forEach((p) => p.split('').forEach((c) => set.add(c)));
...
console.log(input.reduce((count, group) => {
    const set = new Set();
    group.forEach((p) => p.split('').forEach((c) => set.add(c)));
    const allAnswered = [...set].reduce<number>(
        (count, answer) => 
            count + 
            (group.every((p) => !!p.split('').find((c) => answer === c)) ? 1 : 0)
        , 0)
    return count + allAnswered;
}, 0))
