...
const holderMap = Object.fromEntries(
    input.map((s) => {
        const [ holder, rawContents ] = s.split('bags contain').map((t) => t.trim());
        const contents = 
            rawContents
                .split(',')
                .map((t) => t.split('bag')[0].trim())
                .map((t) => t === 'no other' ? ['0'] : u.splitOnce(t, ' '))
                .map(([countStr, ...rest]) => [parseInt(countStr), ...rest] as const)
        return [ holder, contents];
    })
);

const doesHoldGold = (color) => 
    color === 'shiny gold' 
    || !!holderMap[color].find(([ count, type ]) => count !== 0 && doesHoldGold(type));

console.log(
    Object.keys(holderMap)
        .filter((type) => type !== 'shiny gold')
        .reduce((count, type) => (doesHoldGold(type) ? count + 1 : count), 0)
);

const bagCount = (color) => {
    const counts = holderMap[color];
    if (counts[0][0] == 0) return 1;
    return 1 + counts.reduce((sum, [ count, type ]) => sum + count * bagCount(type), 0);
}

console.log(bagCount('shiny gold') - 1)